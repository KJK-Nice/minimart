...
func (u *merchantOrderUsecase) GetMerchantStats(ctx context.Context, merchantID uuid.UUID) (*MerchantOrderStats, error) {
	...
	for _, ord := range orders {
		...
		case order.OrderStatusPending:
			stats.PendingOrders++
			stats.PendingRevenue += ord.TotalAmount().AmountSatoshis()
		...
		case order.OrderStatusCompleted:
			...
			stats.CompletedRevenue += ord.TotalAmount().AmountSatoshis()
		...
		stats.TotalRevenue += ord.TotalAmount().AmountSatoshis()
	}
